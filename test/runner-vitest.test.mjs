...
import { parseReport, runVitest } from '../src/probe/runner-vitest.mjs';
...
describe('runVitest budget', () => {
  it('kills a process that exceeds the wall-clock budget and reports timeout', async () => {
    const { run } = await runVitest({ projectDir: process.cwd(), files: [], budgetMs: 300, command: [process.execPath, '-e', 'setInterval(() => {}, 1000)'] });
    expect(run.outcome).toBe('timeout');
  }, 5000);

  it('reports error when the command produces no report', async () => {
    const { run, loadMessage } = await runVitest({ projectDir: process.cwd(), files: [], budgetMs: 5000, command: [process.execPath, '-e', 'console.error("boom"); process.exit(1)'] });
    expect(run.outcome).toBe('error');
